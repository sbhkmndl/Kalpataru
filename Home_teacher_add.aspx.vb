...
Partial Class Home_teacher_add
    ...
    Protected Sub ImageButton1_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton1.Click
        ...
        Try
            con.Open()
            cmd1 = New SqlCommand("select MAX(cast(sl as int)) from hometec", con)
            dr1 = cmd1.ExecuteReader
            If dr1.Read Then
                n = Val(dr1(0).ToString)
                n = n + 1
                str1 = Val(n)
                str2 = "KLPTRU/HTECH/NO/" & n.ToString
            End If
        ...
        Else
            ...
            Try
                ...
                cmd = New SqlCommand("insert into hometec(tid,sl,tname,tgend,tpic,taddr,frmcls,tocls,rate,pno,math,eng,beng,hst,psc,lsc,grg,cems,al,mail,scnc,arts) values(@tid,@sl,@tname,@tgend,@tpic,@taddr,@frmcls,@tocls,@rate,@pno,@math,@eng,@beng,@hst,@psc,@lsc,@grg,@cems,@al,@mail,@scnc,@arts)", con)
    ...
    Protected Sub Button2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button2.Click
        If FileUpload1.HasFile Then
            ...
            filePath = Server.MapPath("~/images/profile_images/hometeacher/" & Convert.ToString(fileName))
            FileUpload1.SaveAs(filePath)
            fileName = "~/images/profile_images/hometeacher/" & fileName
    ...
    Protected Sub ImageButton2_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImageButton2.Click
        ...
        If Session("insupd") = "upd" Then
            Try
                ...
                If upurl.Text <> "" Then
                    cmd = New SqlCommand("update hometec set   tname =@tname, tgend =@tgend, tpic =@tpic, taddr =@taddr, frmcls =@frmcls, tocls =@tocls, rate =@rate, pno =@pno, math =@math, eng =@eng, beng =@beng, hst =@hst, psc =@psc, lsc =@lsc, grg =@grg, cems =@cems, al =@al, mail =@mail, scnc =@scnc, arts =@arts where tid='" + ambid + "'", con)
                Else
                    cmd = New SqlCommand("update hometec set   tname =@tname, tgend =@tgend, taddr =@taddr, frmcls =@frmcls, tocls =@tocls, rate =@rate, pno =@pno, math =@math, eng =@eng, beng =@beng, hst =@hst, psc =@psc, lsc =@lsc, grg =@grg, cems =@cems, al =@al, mail =@mail, scnc =@scnc, arts =@arts where tid='" + ambid + "'", con)
                End If
                cmd.Parameters.AddWithValue("@tname", TextBox1.Text.Trim())
                cmd.Parameters.AddWithValue("@tgend", genhold)
                If upurl.Text <> "" Then
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Try
            con.Open()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("select * from hometec where tid='" + idhold + "'", con)
