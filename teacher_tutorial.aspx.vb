...
Partial Class teacher_tutorial
            ...
            If dr1ind <> 0 And dr2ind <> 0 Then
                Try
                    ...
                    If dropsel1 = "math" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (math='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "eng" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (eng='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "beng" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (beng='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "hst" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (hst='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "psc" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (psc='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "lsc" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (lsc='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "grg" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (grg='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "cems" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (cems='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "al" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (al='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    ElseIf dropsel1 = "scnc" Then
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (scnc='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)

                    Else
                        cmd = New SqlCommand("select tid, tname, tgend, rte, tme, rloc from tutorial where (arts='Y') and ((cast(frmcls as int))<= '" + dropsel2 + "') and ((cast(tocls as int)) >= '" + dropsel2 + "')", con)
    ...
    Protected Sub GridView1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles GridView1.SelectedIndexChanged
        ...
        Label28.Text = row.Cells(2).Text
        Label29.Text = row.Cells(1).Text
        Label30.Text = row.Cells(3).Text
        Label31.Text = row.Cells(4).Text
        Label32.Text = row.Cells(5).Text
        Label33.Text = row.Cells(6).Text
        Label34.Text = DropDownList2.Text
        Label35.Text = DropDownList1.Text
    End Sub
    ...
    Protected Sub Button5_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button5.Click
        ...
        Try
            con.Open()
            Dim cmd As SqlCommand
            cmd = New SqlCommand("select * from tutorial where tid='" + Label29.Text + "'", con)
            Dim dr As SqlDataReader
            dr = cmd.ExecuteReader
            If dr.Read Then
                Session("tutortid") = dr(0).ToString
                Session("tutorname") = dr(2).ToString
                Session("tutorpic") = dr(4).ToString
                Session("tutormob") = dr(11).ToString
                Session("tutorgend") = dr(3).ToString
                Session("mail") = dr(21).ToString
                rte = dr(10).ToString
            End If
        ...
        Try
            ...
            perposee = "Tutorial Teacher for class " & Label34.Text & " and subject is " & Label35.Text
            ...
            Session("blno") = str2
            Session("ordtyp") = "tutorialtech"
            Session("orddat") = DateTime.Now.ToString("dd-MM-yyyy")
            Session("ordtme") = DateTime.Now.ToString("hh:mm tt")
            Session("Price") = rte
            Session("tutorsbj") = Label35.Text
            Session("tutorclss") = Label34.Text
            Session("tutorstdnm") = TextBox15.Text
            Session("tutortime") = Label32.Text.Trim()
            Session("tutorrdloc") = Label33.Text
